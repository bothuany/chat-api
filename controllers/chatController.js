...
const accessChat = asyncHandler(async (req, res) => {
  const { username } = req.body; //username of the user to chat with
  ...
  let userApplicationId = req.user.application.toString();
  if (req.application.id != userApplicationId) {
    console.log("User does not belong to this application");
    return res.sendStatus(400);
  }

  const user = await User.findOne({ username: username, application: req.user.application });
